	...
	P1 = seqOpsNode(
			DoWhile(
				SeqOps(
					...
					InstrOps(	# strexeq r1, r2, [lock]
							TempReg('val_z') << Register('z'),
							CondOps( TempReg('val_z') == 1,
								SeqOps(
									TempReg('val') << Register('r2'),
									OprStoreCond(TempReg('res'), Location('lock'), TempReg('val')),
									Register('r1') << TempReg('res')
				...
				((Register('r1') == 1) | (Register('r1') == 0)) & 
					((TempReg('res') == 0) | (TempReg('res') == 1)) ,
				(Register('r1') == 1) & (TempReg('res') == 1)
			),
			InstrOps(
				hFW.DMB()
				),
